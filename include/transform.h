...
inline Quat Transform::quaternion_from_vector(const Vec3& v) {
  return Quat(0.0f, v.x, v.y, v.z);
}

inline Quat Transform::quaternion_from_rotation(const Vec3& axis,
                                                const float angle) {
  ...
  return Quat(cosine, vector_component.x, vector_component.y,
              vector_component.z);
}

inline Vec3 Transform::vector_from_quaternion(const Quat& q) {
